...
export default function A4WordComposer() {
  ...
  async function exportPDF() {
      ...
      pdf.save("document_composer.pdf");
  ...
  async function exportImages(type) {
      ...
      const imageType = type === 'png' ? 'image/png' : 'image/jpeg';
      ...
      for (let i = 0; i < pageElements.length; i++) {
          ...
          const imgData = canvas.toDataURL(imageType, type === 'jpeg' ? 0.9 : 1.0);
          
          const link = document.createElement('a');
          link.href = imgData;
          link.download = `document_page_${i + 1}.${extension}`;
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      files.forEach(file => {
        ...
        if (fileType.startsWith("image/")) {
          // Template drop επιτρέπεται μόνο αν υπάρχει DOCX
          if (docHtml) {
              handleTemplate(file);
          }
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-5 flex-wrap mb-6 bg-white p-4 rounded shadow">
        ...
        {/* 2. 📄 Template File (DISABLED/ENABLED) */}
        <label 
